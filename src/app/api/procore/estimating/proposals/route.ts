...
function asArray(payload: unknown): unknown[] {
  ...
  const candidates = [record.data, record.proposals, record.items, record.results];
  ...
  try {
    ...
    for (const host of hostCandidates.candidates) {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          {
            error: `Proposals API error ${response.status}`,
            details: errorText,
            host,
          },
          { status: response.status }
      ...
      const proposals = asArray(payload);

      return NextResponse.json({
        success: true,
        source: "estimating.proposals",
        companyId,
        bidBoardProjectId,
        page,
        perPage,
