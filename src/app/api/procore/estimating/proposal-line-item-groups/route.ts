...
export async function POST(request: Request) {
  try {
    ...
    const requestedBaseUrl = String(body.baseUrl || process.env.PROCORE_ESTIMATING_API_URL || DEFAULT_ESTIMATING_BASE_URL).trim();
    ...
    for (const host of hostCandidates.candidates) {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          {
            error: `Line item groups API error ${response.status}`,
            details: errorText,
            host,
          },
          { status: response.status }
      ...
      const lineItemGroups = asArray(payload);

      return NextResponse.json({
        success: true,
        source: "estimating.proposal_line_item_groups",
        companyId,
        bidBoardProjectId,
        proposalId,
        page,
    ...
    return NextResponse.json(
      {
        error: "Failed to fetch proposal line item groups",
